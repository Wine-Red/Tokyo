      ...
      <div v-for="(region, index) in regions" :key="region.id" class="flex flex-col gap-3 animate-fade-in-up" :class="`stagger-${index + 1}`">
        ...
        <div class="flex items-center justify-between border-b-2 border-black pb-2">
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 flex items-center justify-center shrink-0">
              <img :src="getImageUrl(region.icon)" :alt="region.name" class="max-w-full max-h-full object-contain" />
            </div>
        ...
        <div class="grid grid-cols-2 gap-4 md:gap-6">
          ...
          >
            ...
            <img :src="getImageUrl(team.logo)" :alt="team.name" class="w-10 h-10 md:w-12 md:h-12 object-contain transition-transform duration-300 group-hover:scale-105 shrink-0" />
